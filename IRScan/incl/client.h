...
public:
	...
	//获取扫描ID
	int get_scanid(const std::string &card_id, std::string &scan_id, const std::string &user);
	//发送图片
	...
	//获取图片
	bool get_png(const std::string &scan_id, const std::string &png_id, unsigned short *data);
	//发送结果图片
	bool send_result(const std::string &scan_id, unsigned short *data, int len, const std::string &user);
